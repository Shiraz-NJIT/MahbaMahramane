...
namespace Njit.ImageListView
{
    ...
    public static partial class ImageListViewRenderers
    {
        ...
        public class NoirRenderer : ImageListView.ImageListViewRenderer
        {
            ...
            public override void DrawItem(Graphics g, ImageListViewItem item, ItemState state, Rectangle bounds)
            {
                ...
                if (ImageListView.View == View.Details)
                {
                    ...
                    {
                        if (ImageListView.SortColumn == column.Type && ImageListView.SortOrder != SortOrder.None &&
            ...
            public override void DrawColumnHeader(Graphics g, ImageListView.ImageListViewColumnHeader column, ColumnState state, Rectangle bounds)
            {
                ...
                // Draw the sort arrow
                int textOffset = 4;
                if (ImageListView.SortOrder != SortOrder.None && ImageListView.SortColumn == column.Type)
                {
                    Image img = null;
            ...
            public override void DrawPane(Graphics g, ImageListViewItem item, Image image, Rectangle bounds)
            {
                ...
                if (item != null && image != null)
                {
                    ...
                    // Item text
                    if (ImageListView.Columns[ColumnType.Name].Visible && bounds.Height > 0)
                    {
                        int y = Utility.DrawStringPair(g, bounds, "", item.Text, ImageListView.Font,
                    ...
                    // File type
                    if (ImageListView.Columns[ColumnType.FileType].Visible && bounds.Height > 0 && !string.IsNullOrEmpty(item.FileType))
                    {
                        ...
                        using (Brush bText = new SolidBrush(Color.White))
                        {
                            int y = Utility.DrawStringPair(g, bounds, ImageListView.Columns[ColumnType.FileType].Text + ": ",
                                item.FileType, ImageListView.Font, bCaption, bText);
                            bounds.Y += y;
                            bounds.Height -= y;
                        }
                    }
        ...
        public class TilesRenderer : ImageListView.ImageListViewRenderer
        {
            ...
            public override Size MeasureItem(Njit.ImageListView.View view)
            {
                if (view == Njit.ImageListView.View.Thumbnails)
                {
            ...
            public override void DrawItem(Graphics g, ImageListViewItem item, ItemState state, Rectangle bounds)
            {
                if (ImageListView.View == Njit.ImageListView.View.Thumbnails)
                {
                    Size itemPadding = new Size(4, 4);
        ...
        public class XPRenderer : ImageListView.ImageListViewRenderer
        {
            ...
            public override Size MeasureItem(Njit.ImageListView.View view)
            {
                ...
                if (view == Njit.ImageListView.View.Details)
                    return base.MeasureItem(view);
                else
                {
                    // Calculate item size
